...
class FileLockerApp:
    def __init__(self, root):
        ...
        step1_frame = ttk.LabelFrame(
            main_content, text="步骤 1: 选择目标文件", padding="15 10"
        )
        ...
        self.entry_path = ttk.Entry(
            input_frame, textvariable=self.file_path, font=self.console_font
        )
        self.entry_path.pack(side="left", fill="x", expand=True, padx=(0, 10))

        btn_browse = ttk.Button(
            input_frame, text="📁 浏览...", command=self.browse_file
        )
        btn_browse.pack(side="right")

        # --- 区域 2: 密码输入与操作 ---
        step2_frame = ttk.LabelFrame(
            main_content, text="步骤 2: 安全凭证与操作", padding="15 10"
        )
        step2_frame.pack(fill="x", pady=(0, 15))

        ttk.Label(step2_frame, text="输入密码:", style="Normal.TLabel").pack(anchor="w")
        self.entry_password = ttk.Entry(
            step2_frame, show="•", font=self.normal_font, width=40
        )
        ...
        # --- 区域 3: 操作日志/进度展示 ---
        log_frame_container = ttk.LabelFrame(
            main_content, text="操作日志与进度监控", padding="10"
        )
